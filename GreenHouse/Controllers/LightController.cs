...
namespace GreenHouse.Controllers
{
    public class LightController : IController
    {
        ...
        public LightController(RegulatorMap rm, SensorMap sm, GrowthPlan gp)
        {
            this.rm = rm;
            this.sm = sm;
            this.gp = gp;
        ...
        {
            sm.mapOfLightSensors.Add(loc, (LightSensor)s);
        }
        ...
        {
            int i = 0;//Counter
            if (recievedValues == null) recievedValues = new double[sm.mapOfLightSensors.Count];
            foreach (LightSensor sensor in sm.mapOfLightSensors.Values)
            {
        ...
        {
            int regQuantity = rm.mapOfLightRegulators.Count();//Количество регуляторов
            int sensQuantity = sm.mapOfLightSensors.Count();//Количество сенсоров
            ...
            foreach (Location sensLoc in sm.mapOfLightSensors.Keys)//Weight Coefficients counting
            {
                foreach (Location regLoc in rm.mapOfLightRegulators.Keys)
                {
            ...
            ParamValues.Corridor neededValues = gp.getLight();//Corridor
            ...
            Vector<double> H = Vector<double>.Build.Dense(regQuantity, 20);//Step
            regValues = HookJivsMethod(regValues, sensValues, weightCoefficients, H, 2.5, 0.1, averageValue, neededValues);
           /* if (powerValues == null) powerValues = new double[rm.mapOfLightRegulators.Count];
        ...
        {
            int i = 0;
            foreach (LightRegulator regulator in rm.mapOfLightRegulators.Values)
            {
